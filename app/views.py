...
def signup(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            login(request, user)
            Profile.objects.create(nickname=username, usd_balance=round(random.uniform(100.0, 100000.0), 2),
...
def place_order(request):
    try:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                pending_orders_btc = sum(
                    Order.objects.filter(profile=current_user, order_status='OPENED', order_type='SELL').values_list(
                        'quantity',
                        flat=True))  # sum of current user totals btc pending in sell orders
                pending_orders_usd = sum([item.price * item.quantity for item in
                                          Order.objects.filter(profile=current_user, order_status='OPENED',
                ...
                if order_type == 'SELL' and quantity < customer.btc_balance and quantity < (
                    ...
                    while new_order.order_status == 'OPENED':
                        ...
                        if new_order.quantity < matching_buy_order.quantity and new_order.order_status == 'OPENED':
                            ...
                            matching_buy_order.quantity = matching_buy_order.quantity - new_order.quantity
                            matching_buy_order.save(
                                update_fields=['quantity'])

                            y = matching_buy_order.profile_id
                            customer.usd_balance = (
                                    customer.usd_balance + new_order.quantity * new_order.price)
                            ...
                            Order.objects.create(quantity=new_order.quantity, price=new_order.price, profile=buyer,
                        ...
                        if new_order.quantity > matching_buy_order.quantity and new_order.order_status == 'OPENED':
                            ...
                            filled_order = Order.objects.create(quantity=matching_buy_order.quantity,
                                                                price= matching_buy_order.price, profile=customer,
                                                                order_type='SELL', order_status='CLOSED')
                            y = matching_buy_order.profile_id
                            
                            
                            customer.usd_balance = (
                                    customer.usd_balance + filled_order.quantity * filled_order.price)
                    ...
                    while new_order.order_status == 'OPENED':
                        ...
                        if round(new_order.quantity, 1) == round(matching_sell_order.quantity, 1):
                            ...
                            customer.usd_balance = customer.usd_balance - quantity * new_order.price
                            customer.btc_balance += quantity
                            customer.save(
                                update_fields=['usd_balance', 'btc_balance'])
                            seller = Profile.objects.get(nickname=y)
                            seller.usd_balance = seller.usd_balance + quantity * matching_sell_order.price
                        ...
                        if new_order.quantity < matching_sell_order.quantity and new_order.order_status == 'OPENED':
                            ...
                            matching_sell_order.save(
                                update_fields=['quantity'])
                            seller_nickname = matching_sell_order.profile_id  #equal to seller name
                            customer.usd_balance = customer.usd_balance - new_order.quantity * new_order.price
                            customer.btc_balance += new_order.quantity
                            customer.save(
                                update_fields=['usd_balance', 'btc_balance'])
                            seller = Profile.objects.get(
                                nickname=seller_nickname)
                            filled_sell = Order.objects.create(quantity=new_order.quantity, price=new_order.price,
                                                               profile=seller,
                                                               order_type='SELL', order_status='CLOSED')
                        ...
                        if new_order.quantity > matching_sell_order.quantity and new_order.order_status == 'OPENED':
                            ...
                            seller.usd_balance = seller.usd_balance + \
                                                 matching_sell_order.quantity * matching_sell_order.price
                    ...
                    if quantity < 0:
                        messages.success(request, 'You can not fill an order with negative quantity! Change your amount.')
                    else: 
        ...
        elif request.user.is_authenticated and Profile.objects.filter(nickname=current_user).exists() is False:
            messages.warning(
                request,
...
def overview(request):
    try:
        ...
        usd_spent = sum([item.price * item.quantity for item in
                         Order.objects.filter(profile=current_user, order_status='CLOSED',
                                              order_type='BUY')])  # spent money usd trough buy orders
        btc_acquired = sum(
            Order.objects.filter(profile=current_user, order_status='CLOSED', order_type='BUY').values_list('quantity',
                                                                                                            flat=True))
        btc_sold = sum(
            Order.objects.filter(profile=current_user, order_status='CLOSED', order_type='SELL').values_list('quantity',
                                                                                                             flat=True))
        usd_obtained = sum([item.price * item.quantity for item in
                            Order.objects.filter(profile=current_user, order_status='CLOSED', order_type='SELL')])

        initial_btc_balance = round((btc_balance + btc_sold - btc_acquired), 2)
        initial_usd_balance = round(
            (usd_balance + usd_spent - usd_obtained), 2)
        btc_gains_prc = str(
            round((btc_balance - initial_btc_balance) / initial_btc_balance * 100, 2)) + ' %'
        usd_gains_prc = str(
            round((usd_balance - initial_usd_balance) / initial_usd_balance * 100, 2)) + ' %'
        usd_gains = round((usd_balance - initial_usd_balance), 2)
        btc_gains = round((btc_balance - initial_btc_balance), 2)

        open_orders = Order.objects.filter(
            order_status='OPENED', profile=current_user)
        closed_orders = Order.objects.filter(
            order_status='CLOSED', profile=current_user)

        return render(request, 'overview.html',
                      {'open_orders': open_orders, 'closed_orders': closed_orders, 'usd_gains_prc': usd_gains_prc,
                       'btc_gains_prc': btc_gains_prc, 'usd_gains': usd_gains, 'btc_gains': btc_gains,
                       'initial_btc': initial_btc_balance, 'initial_usd': initial_usd_balance,
